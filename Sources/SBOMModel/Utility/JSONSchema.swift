...
package struct JSONSchema {
    ...
    package func validate(_ jsonObject: Any) throws {
        if isJSONLDDocument(jsonObject) {
            try validateJSONLDDocument(jsonObject as! [String: Any])
        } else {
    ...
    private func validateJSONLDDocument(_ rootDict: [String: Any]) throws {
        try validateJSONLDContext(rootDict[JSONLDKeys.context])
    ...
    private func validateRequiredProperties(_ object: [String: Any], required: [String], path: String) throws {
        for property in required {
            if isSPDXPropertySatisfied(property: property, in: object) {
                continue
            }
    ...
    private func isSPDXPropertySatisfied(property: String, in object: [String: Any]) -> Bool {
        ...
        return property == SPDXKeys.spdxId && object[JSONLDKeys.id] != nil
    }
    ...
    private func resolvePropertySchema(key: String, properties: [String: [String: Any]], object: [String: Any]) -> [String: Any]? {
    ...
    private func validatePattern(_ value: String, pattern: String, path: String) throws {
        ...
        if pattern == "^(?!_:).+:.+" && path.contains("[@graph][") && !path.hasSuffix(".type") && !value.hasPrefix("_:") && allowSimpleId {
            ...
            if let idRegex = try? NSRegularExpression(pattern: simpleIdPattern),
               idRegex.firstMatch(in: value, options: [], range: NSRange(location: 0, length: value.utf16.count)) != nil {
                return // Valid simple identifier reference
            }
            
            if let uuidRegex = try? NSRegularExpression(pattern: uuidPattern),
               uuidRegex.firstMatch(in: value, options: [], range: NSRange(location: 0, length: value.utf16.count)) != nil {
    ...
    private func validateAllOf(_ value: Any, schemas: [[String: Any]], path: String) throws {
        ...
        if !errors.isEmpty {
            ...
            throw StringError("Value at \(path) does not match all allOf schemas.\nValue: \(valueDesc)\nErrors:\n  \(allErrors)")
        }
    }
    ...
    private func validateArrayConstraints(_ array: [Any], schema: [String: Any], path: String) throws {
        if let minItems = schema["minItems"] as? Int {
            if array.count < minItems {
                throw StringError("Array at \(path) has fewer items than minimum. Expected at least \(minItems), got \(array.count)")
            }
        }
        
        if let maxItems = schema["maxItems"] as? Int {
            if array.count > maxItems {
                throw StringError("Array at \(path) has more items than maximum. Expected at most \(maxItems), got \(array.count)")
            }
        }
    ...
    private func validateUniqueItems(_ array: [Any], path: String) throws {
        ...
        for (index, item) in array.enumerated() {
            ...
            if seen.contains(itemKey) {
                ...
                throw StringError("Array at \(path) contains duplicate items. Duplicate found at index \(index): \(itemDesc)")
            }
            seen.insert(itemKey)
        }
    }
