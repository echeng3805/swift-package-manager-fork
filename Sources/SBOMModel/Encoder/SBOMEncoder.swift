...
package struct SBOMEncoder {
    ...
    package func writeSBOMs(specs: [Spec], outputDir: AbsolutePath) async throws {
        ...
        for spec in specs {
            try await encodeSBOM(spec: spec, outputDir: outputDir)
        }
    }
    ...
    package func encodeSBOMData(spec: SBOMSpec) async throws -> Data {
        ...
        case .cyclonedx, .cyclonedx1:
            data = try await CycloneDXConverter.convertToCycloneDXDocument(from: sbom, spec: spec)
        case .spdx, .spdx3:
            data = try await SPDXConverter.convertToSPDXGraph(from: sbom, spec: spec)
