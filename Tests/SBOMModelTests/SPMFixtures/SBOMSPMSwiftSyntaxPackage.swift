...
extension SBOMTestModulesGraph {
    ...
    static func createSPMSwiftSyntaxPackage() -> (
        ...
        let swiftCompilerPluginMessageHandlingModule = self.createSwiftModule(name: "SwiftCompilerPluginMessageHandling")
        ...
        let swiftSyntaxMacrosGenericTestSupportModule = self.createSwiftModule(name: "SwiftSyntaxMacrosGenericTestSupport")
        ...
        let package = self.createPackage(
            ...
            products: [
                ...
                swiftLibraryPluginProviderProduct, swiftSyntaxMacrosGenericTestSupportProduct, swiftSyntaxMacrosTestSupportProduct
            ]
        )
        
        // MARK: - Create resolved modules with dependencies
        
        let resolvedSwiftSyntaxCShimsModule = self.createResolvedModule(packageIdentity: identity, module: swiftSyntaxCShimsModule)
        let resolvedSwiftSyntax509Module = self.createResolvedModule(packageIdentity: identity, module: swiftSyntax509Module)
        let resolvedSwiftSyntax510Module = self.createResolvedModule(packageIdentity: identity, module: swiftSyntax510Module)
        let resolvedSwiftSyntax600Module = self.createResolvedModule(packageIdentity: identity, module: swiftSyntax600Module)
        let resolvedSwiftSyntax601Module = self.createResolvedModule(packageIdentity: identity, module: swiftSyntax601Module)
        let resolvedSwiftSyntax602Module = self.createResolvedModule(packageIdentity: identity, module: swiftSyntax602Module)
        let resolvedSwiftSyntax603Module = self.createResolvedModule(packageIdentity: identity, module: swiftSyntax603Module)
        let resolvedSwiftSyntaxGenericTestSupportModule = self.createResolvedModule(packageIdentity: identity, module: swiftSyntaxGenericTestSupportModule)
        let resolvedSwiftLibraryPluginProviderCShimsModule = self.createResolvedModule(packageIdentity: identity, module: swiftLibraryPluginProviderCShimsModule)
        ...
        let resolvedSwiftSyntaxProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftSyntaxProduct, modules: IdentifiableSet([resolvedSwiftSyntaxModule]))
        let resolvedSwiftDiagnosticsProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftDiagnosticsProduct, modules: IdentifiableSet([resolvedSwiftDiagnosticsModule]))
        let resolvedSwiftParserProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftParserProduct, modules: IdentifiableSet([resolvedSwiftParserModule]))
        let resolvedSwiftBasicFormatProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftBasicFormatProduct, modules: IdentifiableSet([resolvedSwiftBasicFormatModule]))
        let resolvedSwiftOperatorsProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftOperatorsProduct, modules: IdentifiableSet([resolvedSwiftOperatorsModule]))
        let resolvedSwiftParserDiagnosticsProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftParserDiagnosticsProduct, modules: IdentifiableSet([resolvedSwiftParserDiagnosticsModule]))
        let resolvedSwiftSyntaxBuilderProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftSyntaxBuilderProduct, modules: IdentifiableSet([resolvedSwiftSyntaxBuilderModule]))
        let resolvedSwiftIDEUtilsProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftIDEUtilsProduct, modules: IdentifiableSet([resolvedSwiftIDEUtilsModule]))
        let resolvedSwiftIfConfigProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftIfConfigProduct, modules: IdentifiableSet([resolvedSwiftIfConfigModule]))
        let resolvedSwiftLexicalLookupProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftLexicalLookupProduct, modules: IdentifiableSet([resolvedSwiftLexicalLookupModule]))
        let resolvedSwiftRefactorProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftRefactorProduct, modules: IdentifiableSet([resolvedSwiftRefactorModule]))
        let resolvedSwiftSyntaxMacrosProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftSyntaxMacrosProduct, modules: IdentifiableSet([resolvedSwiftSyntaxMacrosModule]))
        let resolvedSwiftSyntaxMacroExpansionProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftSyntaxMacroExpansionProduct, modules: IdentifiableSet([resolvedSwiftSyntaxMacroExpansionModule]))
        let resolvedSwiftCompilerPluginProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftCompilerPluginProduct, modules: IdentifiableSet([resolvedSwiftCompilerPluginModule]))
        let resolvedSwiftCompilerPluginMessageHandlingProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftCompilerPluginMessageHandlingProduct, modules: IdentifiableSet([resolvedSwiftCompilerPluginMessageHandlingModule]))
        let resolvedSwiftLibraryPluginProviderProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftLibraryPluginProviderProduct, modules: IdentifiableSet([resolvedSwiftLibraryPluginProviderModule]))
        let resolvedSwiftSyntaxMacrosGenericTestSupportProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftSyntaxMacrosGenericTestSupportProduct, modules: IdentifiableSet([resolvedSwiftSyntaxMacrosGenericTestSupportModule]))
        let resolvedSwiftSyntaxMacrosTestSupportProduct = self.createResolvedProduct(packageIdentity: identity, product: swiftSyntaxMacrosTestSupportProduct, modules: IdentifiableSet([resolvedSwiftSyntaxMacrosTestSupportModule]))
        ...
        let resolvedPackage = self.createResolvedPackage(
            ...
            modules: IdentifiableSet([
                ...
                resolvedSwiftSyntax603Module, resolvedSwiftSyntaxGenericTestSupportModule, resolvedSwiftLibraryPluginProviderCShimsModule,
